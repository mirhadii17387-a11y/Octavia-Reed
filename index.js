// index.js
// Aman_BilalShah - SMS Proxy API (Full A to Z - Numbers & SMS Reports)
...
const DEFAULT_PHPSESSID = process.env.DEFAULT_PHPSESSID || "9f6cem9j7ra7ackpr45hd3ghuc";
...
// ==================== FETCH HELPER WITH DECOMPRESSION ====================
...
app.get("/", async (req, res) => {
  ...
  try {
    ...
    console.log(`[${new Date().toISOString()}] [${type.toUpperCase()}] Request | Session: ${session.substring(0, 15)}... | Date: ${today}`);
    ...
    try {
      ...
      parsedData = { raw: rawData.substring(0, 300) + "..." };
    }
    ...
    console.error(`[${new Date().toISOString()}] Error [${req.query.type}]:`, err.message);
    res.status(502).json({
      success: false,
      error: "Failed to fetch data from SMS panel",
      details: process.env.NODE_ENV === "production" ? "Proxy internal error" : err.message,
...
app.listen(PORT, () => {
  ...
  console.log(`   Default Session: ${DEFAULT_PHPSESSID.substring(0, 12)}...`);
});
