// index.js
// Aman_BilalShah - SMS Proxy API (Full A to Z - Updated with iDisplayLength=-1)
...
app.get("/", async (req, res) => {
  ...
  try {
    ...
    if (type === "numbers") {
      ...
      // Updated with your latest curl (iDisplayLength=-1)
    ...
    console.log(`[${new Date().toISOString()}] [${type.toUpperCase()}] Request | Session: ${session.substring(0, 15)}... | Date: ${today} | Length: -1`);
    ...
    try {
      ...
      parsedData = { error: "Failed to parse JSON", raw: rawData.substring(0, 500) };
    }
